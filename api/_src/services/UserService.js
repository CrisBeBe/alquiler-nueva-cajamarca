...
class UserService {
  ...
  async deleteAccount(userId) {
    ...
    try {
      ...
      await UserRepository.update(userId, { estado: 'inactivo' }, t);
      
      // Borrado lógico de todos sus anuncios
      await AnuncioRepository.deactivateAllByUserId(userId, t);
      
      await t.commit();
      return { message: 'Cuenta desactivada y anuncios retirados correctamente' };
    } catch (error) {
      await t.rollback();
      throw { message: 'Error al eliminar la cuenta', error, status: 500 };
    }
  }
